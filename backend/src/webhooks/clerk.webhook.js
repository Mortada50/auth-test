...
// ⚠️ يجب raw body قبل أي JSON parser
router.post(
  "/clerk",
  ...
  async (req, res) => {
    ...
    try {
      ...
      // نرجع 200 حتى لا يعيد Clerk الإرسال بشكل متكرر
      // لكن نسجل الخطأ لمراجعته
...
async function handleUserCreated(data) {
  ...
  if (!email || !role) {
    throw new Error(`بيانات ناقصة: email=${email}, role=${role}`);
  }
  ...
  switch (role) {
    case ROLES.DOCTOR:
      mongoDoc = await Doctor.create({
        ...
        clinicAddress: { city: unsafe_metadata.clinicCity },
        medicalLicense: unsafe_metadata.medicalLicense,
      });
      break;
    ...
    case ROLES.PHARMACY:
      mongoDoc = await Pharmacy.create({
        ...
        pharmacyAddress: { city: unsafe_metadata.city },
        pharmacyLicense: unsafe_metadata.pharmacyLicense,
      });
      break;
    ...
    default:
      throw new Error(`Role غير معروف: ${role}`);
  }

  // ── بعد الحفظ في MongoDB: حدّث publicMetadata في Clerk ──
  const status = role === ROLES.PATIENT ? STATUS.ACTIVE : STATUS.PENDING;
...
async function handleUserUpdated(data) {
  ...
  // إذا تغير الإيميل، نحدثه في MongoDB
...
async function handleUserDeleted(data) {
  ...
  // حذف من كل الجداول (soft delete أو hard delete)
